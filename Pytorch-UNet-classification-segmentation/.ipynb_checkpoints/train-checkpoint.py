        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp):
                    masks_pred, class_pred = net(images)
#                     print(masks_pred.shape)
                    seg_loss = 0.5*(criterion_segmentation(masks_pred, true_masks))
                    clas_loss = 0.5*(criterion_classification(class_pred, true_out))
                    loss =  seg_loss + clas_loss
#                     print(seg_loss, clas_loss)
